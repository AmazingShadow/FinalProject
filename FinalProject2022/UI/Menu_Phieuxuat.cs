...
{
    public partial class Menu_Phieuxuat : UserControl
    {
        QUANLYSIEUTHIEntities2 quanlysieuthi = new QUANLYSIEUTHIEntities2();
        string opt = "";

        public Menu_Phieuxuat()
        {
            InitializeComponent();
        }
        ...
        {
            txt_maphieuxuat.Text = dt_ngayxuat.Text = txt_lydo.Text = cb_diadiem.Text = txt_manhanvien.Text = "";
        }

        // Function này sẽ lấy dữ liệu phiếu xuất từ cơ sở dữ liệu khi form được load.
        private void Menu_Phieuxuat_Load(object sender, EventArgs e)
        {
            pHIEUXUATBindingSource.DataSource = quanlysieuthi.PHIEUXUATs.ToList();
        }

        // Function này thực hiện chức năng thêm phiếu xuất kho vào cơ sở dữ liệu.
        private void addPhieuxuathang()
        {
            // Tạo ra một đối tượng để lấy dữ liệu từ input
            PHIEUXUAT phieuxuat = new PHIEUXUAT
            {
                MAPHIEUXUAT = txt_maphieuxuat.Text.Trim(),
                NGAYXUAT = DateTime.Parse(dt_ngayxuat.Text.Trim()),
                LYDOXUAT = txt_lydo.Text.Trim(),
                DIADIEMXUAT = cb_diadiem.Text.Trim(),
                MANHANVIEN = txt_manhanvien.Text.Trim()
            };

            // Thực hiện lưu dữ liệu vừa nhập vào cơ sở dữ liệu
            quanlysieuthi.PHIEUXUATs.Add(phieuxuat);
            quanlysieuthi.SaveChanges();
        }

        // Function này thực hiện chức năng chỉnh sửa dữ liệu
        private void updatePhieuxuathang()
        {
            // Tìm kiếm nhân viên kho cần chỉnh sửa.
            PHIEUXUAT phieuxuat = quanlysieuthi.PHIEUXUATs.Where(p => p.MAPHIEUXUAT == txt_maphieuxuat.Text).SingleOrDefault();
            phieuxuat.NGAYXUAT = DateTime.Parse(dt_ngayxuat.Text.Trim());
            phieuxuat.LYDOXUAT = txt_lydo.Text.Trim();
            phieuxuat.DIADIEMXUAT = cb_diadiem.Text.Trim();
            phieuxuat.MANHANVIEN = txt_manhanvien.Text.Trim();
        ...
        // Function này xử lý sự kiện thêm phiếu xuất hàng.
        ...
        private void btn_luu_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (txt_maphieuxuat.Text != "" && dt_ngayxuat.Text != "" && txt_lydo.Text != "" && cb_diadiem.Text != "" && txt_manhanvien.Text != "")
            {
        ...
        private void btn_xoa_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            if (XtraMessageBox.Show("Bạn có muốn xóa hay không?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                quanlysieuthi.PHIEUXUATs.Remove(phieuxuat);
